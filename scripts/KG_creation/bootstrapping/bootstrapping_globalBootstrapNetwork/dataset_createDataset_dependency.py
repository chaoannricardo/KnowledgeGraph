# -*- coding: utf8 -*-
from tqdm import tqdm
import codecs
import numpy as np

''' Configurations '''
MATERIALS_DIR = "C:/Users/User/Desktop/Ricardo/KnowledgeGraph_materials/"
SEED_PATH = MATERIALS_DIR + "data_kg/baiduDatasetTranditional_Cleansed/seed_relations_filtered.csv"
DATA_PATH_LIST = [MATERIALS_DIR + "results_kg/Semiconductor/bootstrapped_relationTriples_ExternalKGFiltered.csv"]

# configuration differs on NODE_SET_TYPE
# for single node
# OUTPUT_DIR = MATERIALS_DIR + "data_kg/baiduDatasetTranditional_GBN_singleNode_dependency/doc_un_1/"
# for whole node set
OUTPUT_DIR = MATERIALS_DIR + "data_kg/baiduDatasetTranditional_GBN_wholeNode_dependency/doc_un_1/"
NODE_SET_TYPE = 1
# 0: single node 1: whole node set

if __name__ == '__main__':
    # open files to write
    file_output_entities = codecs.open(OUTPUT_DIR + "entities.txt", mode="w", encoding="utf8")
    file_output_entities_labels = codecs.open(OUTPUT_DIR + "entity_labels.txt", mode="w", encoding="utf8")
    file_output_labels = codecs.open(OUTPUT_DIR + "labels.txt", mode="w", encoding="utf8")
    file_output_links = codecs.open(OUTPUT_DIR + "links.txt", mode="w", encoding="utf8")
    file_output_pattern_label_vocab = codecs.open(OUTPUT_DIR + "pattern_label_vocab.txt", mode="w", encoding="utf8")
    file_output_pattern_label = codecs.open(OUTPUT_DIR + "pattern_labels.txt", mode="w", encoding="utf8")
    file_output_pattern = codecs.open(OUTPUT_DIR + "patterns.txt", mode="w", encoding="utf8")

    # initiate list to store data
    nodes = []
    node_set_list = []
    seed_node_list = []
    seed_node_set_list = []
    patterns = []

    # read in seed file
    for file_path_index, file_path in enumerate([SEED_PATH] + DATA_PATH_LIST):
        file = codecs.open(file_path, encoding="utf8", mode="r", errors="ignore")

        for lineIndex, line in enumerate(tqdm(file.readlines())):
            edge = line.split("&")[-1].replace("\n", "")
            entity_1 = line.split("&")[0].split("@")[0]
            entity_2 = line.split("&")[0].split("@")[-1]
            node_set = entity_1 + "@" + edge + "@" + entity_2

            # store nodes for single node usage
            for entity in [entity_1, entity_2, edge]:
                if entity not in nodes:
                    nodes.append(entity)
                if file_path == SEED_PATH and entity not in seed_node_list:
                    seed_node_list.append(entity)

            # store node set for whole node usage
            if node_set not in node_set_list:
                node_set_list.append(node_set)

            if file_path == SEED_PATH and node_set not in seed_node_set_list:
                seed_node_set_list.append(node_set)

            try:
                dependency_path_list = "&".join(line.split("&")[:-1]).split("@")
                dependency_path_list[0] = "@Entity"
                dependency_path_list[-1] = "@Entity"
                dependency_path_list[dependency_path_list.index(edge)] = "@Predicate"
                dependency_path = " & ".join(dependency_path_list)

                # store patterns
                if dependency_path not in patterns:
                    patterns.append(dependency_path)

                # write links: source target weight
                if NODE_SET_TYPE == 0:
                    file_output_links.write(
                        "\t".join([str(nodes.index(entity_1)), str(patterns.index(dependency_path)), "1"]) + "\n")
                    file_output_links.write(
                        "\t".join([str(nodes.index(entity_2)), str(patterns.index(dependency_path)), "1"]) + "\n")
                    file_output_links.write(
                        "\t".join([str(nodes.index(edge)), str(patterns.index(dependency_path)), "1"]) + "\n")
                elif NODE_SET_TYPE == 1:
                    file_output_links.write("\t".join(
                        [str(node_set_list.index(node_set)), str(patterns.index(dependency_path)), "1"]) + "\n")

            except ValueError:
                continue

    # write entities & entity_label
    writing_entity_list = nodes if NODE_SET_TYPE == 0 else node_set_list
    for entity in writing_entity_list:
        file_output_entities.write(str(entity) + "\n")
        file_output_entities_labels.write("\t".join([str(writing_entity_list.index(entity)), "0"]) + "\n")

    # write labels
    file_output_labels.write("Entity")

    # write patterns
    for index in range(len(patterns)+1):
        file_output_pattern_label_vocab.write(str(index) + "\n")

    for patternIndex, pattern in enumerate(patterns + patterns):
        file_output_pattern_label.write("\t".join([str(patternIndex), "1"]) + "\n")
        file_output_pattern.write(pattern + "\n")

    # write seeds
    file_seed = codecs.open(OUTPUT_DIR + "seeds.txt", mode="w", encoding="utf8")
    writing_entity_list = seed_node_list if NODE_SET_TYPE == 0 else seed_node_set_list
    for entity in writing_entity_list:
        file_seed.write("\t".join([str(writing_entity_list.index(entity)), "0"]) + "\n")